...
class FoldingCard extends StatefulWidget {
  ...
  final bool isOpen;
  final List<FoldEvent> entries;
  final Function onClick;
  final Duration duration;

  FoldingCard({this.duration, @required this.entries, this.isOpen = false, this.onClick});

  @override
  _FoldingTicketState createState() => _FoldingTicketState();
}

class _FoldingTicketState extends State<FoldingCard> with SingleTickerProviderStateMixin {
  List<FoldEvent> _entries;
  double _ratio = 0.0;
  AnimationController _controller;

  double get openHeight => _entries.fold(0.0, (val, o) => val + o.height) + FoldingCard.padding * 2;
  ...
  Widget _buildEntry(int index) {
    FoldEvent entry = _entries[index];
...
class FoldEvent {
  ...
  FoldEvent({@required this.front, @required this.height, Widget back}) {
    this.back = Transform(
